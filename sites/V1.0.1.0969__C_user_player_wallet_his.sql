-- auto gen by steffan 2018-09-04 19:36:55
-- create by min
CREATE TABLE IF not EXISTS "user_player_wallet_history" (
id serial4 NOT NULL,
...
-----触发器函数
create or replace function process_tg_user_player_wallet() returns trigger as $tg_user_player_wallet$--user_player 钱包余额变化
begin
        ...
        insert into user_player_wallet_history(player_id,wallet_balance_before,wallet_balance_after,create_time,remark)
                    select new.id,old.wallet_balance,new.wallet_balance,now(),inet_client_addr()||':' || inet_client_port() ||','||current_query();
                 end if;
     return new;
...
--------触发器
DROP TRIGGER IF EXISTS "tg_user_player_wallet" ON  "user_player";
