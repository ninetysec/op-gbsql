-- auto gen by bruce 2016-10-12 10:06:26
select redo_sqls($$
    alter table player_withdraw add COLUMN rank_id int4;
    alter table player_transaction add COLUMN rank_id int4;
$$);
COMMENT ON COLUMN player_withdraw.rank_id is '层级ID';
COMMENT ON COLUMN player_transaction.rank_id is '层级ID';
...
CREATE OR REPLACE VIEW "v_player_withdraw" AS
 SELECT t1.id,
   ...
   FROM ((((((((player_withdraw t1
     ...
     LEFT JOIN player_rank t5 ON ((t5.id = t1.rank_id)))
