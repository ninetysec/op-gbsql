-- auto gen by cherry 2017-09-21 19:05:49
--修改统计代理数
...
CREATE OR REPLACE VIEW "v_user_agent_manage" AS
 ...
 SELECT ua.id,
    ...
    ( select count(1) from user_agent uag left join sys_user sus on uag.id=sus.id where sus.status in ('1','3') and (uag.parent_id = ua.id)) AS agent_num,
    pr.rank_name,
