...
CREATE OR REPLACE VIEW "v_player_online" AS
 SELECT s.id,
    ...
    u.channel_terminal,
    s.terminal
   FROM (("gb-site-1".user_player u
     JOIN "gb-site-1".sys_user s ON ((s.id = u.id)))
     LEFT JOIN "gb-site-1".player_game_log pgl ON (((pgl.user_id = s.id) AND ((pgl.session_key)::text = (s.session_key)::text))))
