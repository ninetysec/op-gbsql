-- auto gen by linsen 2018-04-15 21:39:57
-- 注册送规则判断 by kobe
DROP FUNCTION IF EXISTS gamebox_activityhall_rule_regist;
...
--判断信息唯一
	...
	LOOP
		IF (rec_preferential.preferential_code = 'bankcard_unique') THEN
			SELECT bankcard_number WHERE user_id = playerid INTO bankcardnumber;
			IF (bankcardnumber IS NULL) THEN
				RETURN json_build_object('resultcode',ACTIVITY_BANKCARD_ERROR);
			ELSEIF EXISTS (SELECT * FROM user_bankcard WHERE user_id in (SELECT id FROM sys_user WHERE user_type = '24' AND id <> playerid) AND bankcard_number = bancardnumber) THEN
				RETURN json_build_object('resultcode',ACTIVITY_BANKCARD_ERROR);
			END IF;
		ELSEIF (rec_preferential.preferential_code = 'real_name_unique') THEN
			...
			ELSEIF EXISTS (SELECT * FROM sys_user WHERE real_name = realname AND id <> playerid) THEN
				RETURN json_build_object('resultcode',ACTIVITY_REALNAME_ERROR);
			END IF;
		ELSEIF (rec_preferential.preferential_code = 'register_ip_unique') THEN
			...
			ELSEIF EXISTS (SELECT * FROM sys_user WHERE register_ip = registerip AND id <> playerid) THEN
				RETURN json_build_object('resultcode',ACTIVITY_REGISTERIP_ERROR);
			END IF;
		END IF;
	END LOOP;
