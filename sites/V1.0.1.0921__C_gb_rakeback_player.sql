-- auto gen by linsen 2018-07-25 15:41:36
    ...
    SELECT p_bill_id, ut.id, ut.username, ut.rank_id, ut.rank_name, ut.risk_marker, 'pending_lssuing',
      ...
      FROM (
            SELECT player_id,
									 ...
									 topagent_id,
									 topagent_username,
                   CASE WHEN SUM(rakeback) > COALESCE(MIN(rakeback_limit), 0) THEN COALESCE(MIN(rakeback_limit),0) ELSE COALESCE(SUM(rakeback),0) END rakeback, --顺序不能改，因为返水上限可能为NULL
    ...
    SELECT p_bill_id, ut.id, ut.username, ut.rank_id, ut.rank_name, ut.risk_marker,
      ...
      FROM (
            SELECT player_id,
									 ...
									 topagent_id,
									 topagent_username,
                   CASE WHEN SUM(rakeback) >  COALESCE(MIN(rakeback_limit), 0) THEN  COALESCE(MIN(rakeback_limit), 0) ELSE SUM(rakeback) END rakeback, --顺序不能改，因为返水上限可能为NULL
                   MIN(audit_num) audit_num,
                   SUM(effective_transaction) effective_transaction
