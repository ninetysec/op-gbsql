-- auto gen by tom 2016-03-22 17:59:00
...
ALTER FUNCTION "gamebox_rakeback_filter_player"(gradshash "hstore", agenthash "hstore", rec json) OWNER TO "postgres";
...
ALTER FUNCTION "gamebox_rakeback_qualified_player"(gradshash "hstore", agenthash "hstore", back_time timestamp) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."gamebox_rakeback_api_base"(back_time timestamp)
            ...
            gradId = playerIdGradId->playerId;
              ...
              po.effective_trade_amount,
						 ...
						 FROM (SELECT pgo.player_id,
								 ...
								 FROM player_game_order pgo
								WHERE pgo.player_id=playerId::int and to_char(pgo.create_time, 'yyyy-MM-dd') = to_char(back_time, 'yyyy-MM-dd')
...
ALTER FUNCTION "public"."gamebox_rakeback_api_base"(back_time timestamp) OWNER TO "postgres";

COMMENT ON FUNCTION "public"."gamebox_rakeback_api_base"(back_time timestamp) IS 'Lins-返水-各玩家API返水基础表.入口';
...
ALTER FUNCTION "gamebox_rakeback_calculator"(gradshash "hstore", agenthash "hstore", rec json) OWNER TO "postgres";
