-- auto gen by cherry 2017-03-17 11:22:18
	...
	--3、每个代理生成一条主表数据
		...
		select p_year,p_month,topagent_id,topagent_name,agent_id,agent_name,api_id,api_type_id,
			...
			COALESCE((CASE WHEN need_history_count THEN (
				...
				select (COALESCE(payout_amount_history,0)+COALESCE(payout_amount,0)) from agent_rebate_grads arg where rebate_year = p_last_year and rebate_month = p_last_month and agent_id=agent_id and arg.api_id=api_id and arg.api_type_id = api_type_id
...
UPDATE sys_user SET default_locale='zh_CN' where default_locale='' or default_locale is null;