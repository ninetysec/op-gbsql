-- auto gen by fei 2017-12-03 16:07:45
...
--官方玩法
	...
	ELSEIF p_play_code='ssc_sanxing_zhixuan' THEN
			...
			ELSEIF p_bet_code in ('ssc_sanxing_zhixuan_qszh','ssc_sanxing_zhixuan_hszh')  THEN
				UPDATE lottery_bet_order  SET payout =(odd+odd2*array_length(string_to_array(split_part(bet_num, '|',1), ','), 1)+odd3*array_length(string_to_array(split_part(bet_num, '|',1), ','), 1)*array_length(string_to_array(split_part(bet_num, '|',2), ','), 1))*coalesce(multiple,1)/coalesce(bonus_model::INT,1)::NUMERIC(20,3),payout_time = now(), status='2',effective_trade_amount=trunc(bet_amount-bet_amount*coalesce(rebate,0),2) WHERE bet_num like p_winning_num and expect=p_expect AND code=p_code AND status='1' AND play_code= p_play_code AND bet_code=p_bet_code;
				temp_str = substr(p_winning_num,3);
				UPDATE lottery_bet_order  SET payout =(odd2*array_length(string_to_array(split_part(bet_num, '|',1), ','), 1)+odd3*array_length(string_to_array(split_part(bet_num, '|',1), ','), 1)*array_length(string_to_array(split_part(bet_num, '|',2), ','), 1))*coalesce(multiple,1)/coalesce(bonus_model::INT,1)::NUMERIC(20,3),payout_time = now(), status='2',effective_trade_amount=trunc(bet_amount-bet_amount*coalesce(rebate,0),2) WHERE bet_num like temp_str and expect=p_expect AND code=p_code AND status='1' AND play_code= p_play_code AND bet_code=p_bet_code;
				temp_str = '%|' || substr(p_winning_num,7);
				UPDATE lottery_bet_order  SET payout =odd3*array_length(string_to_array(split_part(bet_num, '|',1), ','), 1)*array_length(string_to_array(split_part(bet_num, '|',2), ','), 1)*coalesce(multiple,1)/coalesce(bonus_model::INT,1)::NUMERIC(20,3),payout_time = now(), status='2',effective_trade_amount=trunc(bet_amount-bet_amount*coalesce(rebate,0),2) WHERE bet_num like temp_str and expect=p_expect AND code=p_code AND status='1' AND play_code= p_play_code AND bet_code=p_bet_code;
