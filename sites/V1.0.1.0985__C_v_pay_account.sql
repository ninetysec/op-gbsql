-- auto gen by steffan 2018-09-17 19:11:42
CREATE OR REPLACE VIEW "v_pay_account" AS
 SELECT pa.id,
    ...
    rfs.id AS fee_schema_id,
    rfs.schema_name AS fee_schema_name
   FROM ((pay_account pa
     LEFT JOIN fee_account_relation far ON ((pa.id = far.pay_account_id)))
     LEFT JOIN recharge_fee_schema rfs ON ((rfs.id = far.fee_schema_id)));
