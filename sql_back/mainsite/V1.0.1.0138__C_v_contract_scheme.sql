-- auto gen by tom 2015-12-09 16:06:30
CREATE OR REPLACE VIEW "v_contract_scheme" AS
 SELECT cs.id,
    ...
    ( SELECT min(coa.ratio) AS min
          ...
          WHERE (ca.contract_scheme_id = cs.id)) AS min,
    ( SELECT contract_favourable.id
           FROM contract_favourable
          WHERE ((contract_favourable.contract_scheme_id = cs.id) AND ((contract_favourable.favourable_type)::text = '1'::text))) AS type_1_id,
    ( SELECT contract_favourable.id
           FROM contract_favourable
          WHERE ((contract_favourable.contract_scheme_id = cs.id) AND ((contract_favourable.favourable_type)::text = '2'::text))) AS type_2_id,
    ( SELECT count(1) AS count
