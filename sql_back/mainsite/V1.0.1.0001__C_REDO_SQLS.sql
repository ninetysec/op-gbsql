-- auto gen by Kevice 2015-09-01 11:39
...
begin
  ...
  loop
     ...
     IF regexp_matches(sql_str, 'create sequence \S+') is not null THEN
				obj_name := (regexp_matches(sql_str, 'create sequence (\S+)'))[1];
     ...
     ELSEIF regexp_matches(sql_str, 'alter table \S+ add column \S+') is not null THEN
        tmp := regexp_matches(sql_str, 'alter table (\S+) add column (\S+)');
        obj_name := tmp[1];
				IF position('' in obj_name) = 1 THEN
					obj_name := (regexp_matches(obj_name, 'public\.(\w+)'))[1];
				END IF;
     ...
     ELSEIF regexp_matches(sql_str, 'create index \S+ on \S+') is not null THEN
        obj_name := (regexp_matches(sql_str, 'create index (\S+) on \S+'))[1];
	SELECT count(*) into cnt FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relkind = 'i' AND c.relname = obj_name AND n.nspname = 'public';
     ELSEIF regexp_matches(sql_str, 'alter table \S+ add constraint \S+') is not null THEN
        tmp := regexp_matches(sql_str, 'alter table (\S+) add constraint (\S+)');
        obj_name := tmp[1];
	IF position('' in obj_name) = 1 THEN
		obj_name := (regexp_matches(obj_name, 'public\.(\w+)'))[1];
	END IF;
