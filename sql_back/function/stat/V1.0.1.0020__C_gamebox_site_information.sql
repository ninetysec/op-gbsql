-- auto gen by Lins 2015-12-14 06:48:47
/*
* 收集站点总代.代理.玩家等信息.
* @author:Lins
* @date:2015-12-14
...
END;
$$ language plpgsql;
COMMENT ON FUNCTION gamebox_site_information(
main_url TEXT
,master_urls TEXT
,split TEXT
...
BEGIN
	...
	FOR rec IN
		SELECT * FROM dblink(
		'master'
		,'SELECT gamebox_current_site() as site_id,COUNT(CASE user_type WHEN ''22'' THEN ID END) as top_agent_num
	...
	LOOP
		...
		ELSE
			...
			,site_id,site_name
			,topagent_num,agent_num
			,player_num
			)VALUES(
				rec.operationid,rec.operationname
				,rec.masterid,rec.mastername
				,rec.siteid,rec.sitename
				,rec.top_agent_num,rec.agent_num
				,rec.player_num
			);
