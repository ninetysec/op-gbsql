-- auto gen by cherry 2016-01-27 11:20:56
DROP FUNCTION IF EXISTS f_preferential_deposit(activitymessageid int4, money numeric);

CREATE OR REPLACE FUNCTION "f_preferential_deposit"(activitymessageid int4, money numeric)
...
DROP FUNCTION IF EXISTS f_preferential_regist(activitymessageid int4, playerid int4);
...
--生成玩家优惠记录
	...
	ELSE
		...
		WITH playtransaction AS (
			...
			rec.preferential_value + COALESCE((SELECT wallet_balance FROM user_player WHERE id = playerid),0), 'success',playerid, favorable_id_temp, auditPoints, false, false, 'favourable', activityType, '{"favorableType":"'||activityType||'"}')
...
DROP FUNCTION IF EXISTS f_calculator_profit_loss(playerid int4, starttime timestamp, endtime timestamp);

CREATE OR REPLACE FUNCTION "f_calculator_profit_loss"(playerid int4, starttime timestamp, endtime timestamp)
...
SELECT sum(profit_amount) FROM player_game_order WHERE player_id = playerid AND payout_time >= starttime AND payout_time <= endtime INTO profit_loss_amount;
...
COMMENT ON FUNCTION "f_calculator_profit_loss"(playerid int4, starttime timestamp, endtime timestamp) IS '计算时间范围内某玩家的盈亏情况';


DROP FUNCTION if EXISTS f_calculator_effective_transaction(playerid int4, starttime timestamp, endtime timestamp);



CREATE OR REPLACE FUNCTION "f_calculator_effective_transaction"(playerid int4, starttime timestamp, endtime timestamp)
...
SELECT sum(effective_trade_amount) FROM player_game_order WHERE player_id = playerid AND payout_time >= starttime AND payout_time <= endtime INTO effective_transaction;
...
COMMENT ON FUNCTION "f_calculator_effective_transaction"(playerid int4, starttime timestamp, endtime timestamp) IS '计算时间范围内某玩家的有效交易量';
	...
	loop
      ...
      IF rec_way.preferential_audit = NULL THEN
			...
			WITH playtransaction AS (
				...
				(orderNo, now(), 'favorable', '系统自动计算玩家优惠', rec_way.preferential_value, rec_way.preferential_value + (SELECT wallet_balance FROM user_player WHERE id = rec_apply.user_id), 'success',rec_apply.user_id, favorable_id_temp, auditPoints, false, false, 'favourable', activitytype, '{"favorableType":"'||activitytype||'"}')
