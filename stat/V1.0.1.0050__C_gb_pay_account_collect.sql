-- auto gen by cherry 2016-11-12 20:25:12
...
--v1.02  2016/11/12  Chan     添加过虑条件type=2
*/
DECLARE
	rec 	record;
  rec_ds record;
	...
	LOOP
		...
		FOR rec IN
			...
			FROM dblink(site_url,
				...
				code, deposit_count, deposit_total, deposit_default_count, deposit_default_total,
				effective_minutes, single_deposit_min, single_deposit_max, frozen_time, channel_json,
				full_rank, custom_bank_name, open_acount_name, qr_code_url, remark from pay_account where type=''2''')
