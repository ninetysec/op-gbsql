-- auto gen by cherry 2017-06-06 14:56:40
	...
	SELECT comp_id, master_id, site_id, site_name, ip, port, dbname, username, password
	  FROM dblink(comp_url,
	              'SELECT parent_id comp_id, sys_user_id master_id, d.id site_id, d.name site_name,
	                 ...
	                 FROM sys_site s, sys_datasource d where s.id = d.id and s.belong_to_idc=''A''
	                ORDER BY site_id')
...
COMMENT ON FUNCTION "gb_site_jackpot_api"("comp_url" text, "t_api_id" int4, "t_jackpot_name" text, "t_game_id" text, "static_begin_time" text, "static_end_time" text, "t_static_month" text) IS '彩池贡献金-API';
...
COMMENT ON FUNCTION "gb_site_jackpot_master"("comp_url" text, "t_api_id" int4, "t_jackpot_name" text, "t_game_id" text, "t_static_month" text) IS '彩池贡献金-master';