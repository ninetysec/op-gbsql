-- auto gen by cherry 2016-09-19 11:15:44
 select redo_sqls($$
    ALTER TABLE operation_profile ADD COLUMN static_date date;
		ALTER TABLE operation_profile ADD COLUMN static_time timestamp(6);
		ALTER TABLE operation_profile ADD COLUMN static_time_end timestamp(6);
$$);

COMMENT ON COLUMN operation_profile.static_date IS '统计日期';
COMMENT ON COLUMN operation_profile.static_time IS '统计起始时间';
COMMENT ON COLUMN operation_profile.static_time_end IS '统计截止时间';
COMMENT ON COLUMN operation_profile.statistics_time IS '统计时间——停用';
...
BEGIN
	...
	DELETE FROM operation_profile WHERE site_id = sid AND static_date = end_time;
