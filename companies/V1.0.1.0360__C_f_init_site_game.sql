-- auto gen by cherry 2017-07-14 20:02:37
...
SELECT siteid,a.id, url, parameter, order_num,'normal' from api_type a WHERE NOT EXISTS (select * from site_api_type s WHERE a.id=s.api_type_id and s.site_id=siteid);
GET DIAGNOSTICS v_count = ROW_COUNT;
...
SELECT siteid,id, apiid, game_type, order_num, url, status,api_type_id,support_terminal,code from game g WHERE g.api_id=apiid AND NOT EXISTS (select * from site_game s WHERE g.id=s.game_id and s.site_id=siteid);
GET DIAGNOSTICS v_count = ROW_COUNT;
