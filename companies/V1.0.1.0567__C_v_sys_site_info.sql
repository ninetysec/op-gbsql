-- auto gen by linsen 2018-03-04 10:35:38

DROP VIEW IF EXISTS v_sys_site_info;
CREATE OR REPLACE VIEW v_sys_site_info AS
 SELECT s1.id AS siteid,
    ( SELECT si.value
           FROM site_i18n si
          WHERE si.site_id = s1.id AND si.locale = s1.main_language::bpchar AND si.type::text = 'site_name'::text AND si.module::text = 'setting'::text AND si.key::text = 'name'::text
         LIMIT 1) AS sitename,
    s1.sys_user_id AS masterid,
    sm.username AS mastername,
    sm.user_type AS usertype,
    sm.subsys_code AS subsyscode,
    s2.id AS centerid,
    ( SELECT si.value
        FROM site_i18n si
       WHERE si.site_id = s2.id AND si.locale = s2.main_language::bpchar AND si.type::text = 'site_name'::text AND si.module::text = 'setting'::text AND si.key::text = 'name'::text
       LIMIT 1) AS centername,
    s2.sys_user_id AS operationid,
    sc.username AS operationname,
    sc.user_type AS operationusertype,
    sc.subsys_code AS operationsubsyscode,
    s1.main_currency AS currency,
    s1.timezone AS timezone
   FROM sys_site s1
     LEFT JOIN sys_user sm ON s1.sys_user_id = sm.id,
    sys_site s2
     LEFT JOIN sys_user sc ON s2.sys_user_id = sc.id
