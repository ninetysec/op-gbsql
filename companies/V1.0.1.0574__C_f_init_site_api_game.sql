-- auto gen by linsen 2018-03-14 22:32:51
--根据站点id、apiid更新同步数据 by zain
...
=s.api_type_id and s.site_id=siteid) and a.id=(SELECT api_type_id FROM api_type_relation where api_id=apiid);
GET DIAGNOSTICS v_count = ROW_COUNT;
raise notice '同步site_api_type数量 %', v_count;

INSERT INTO site_api_type_i18n (site_id, api_type_id, name, local, cover)
SELECT siteid, api_type_id, name, local, cover from api_type_i18n a WHERE NOT EXISTS (select * from site_api_type_i18n s WHERE a.api_type_id=s.api_type_id and s.site_id=siteid) and  a.api_type_id=(SELECT api_type_id FROM api_type_relation where api_id=apiid);
GET DIAGNOSTICS v_count = ROW_COUNT;
