-- auto gen by cherry 2016-02-18 09:58:05
...
CREATE VIEW v_sport_recommended as
	 ...
	 (SELECT array_to_json(array_agg(row_to_json(a1))) from (select sport_team_id as name,local as language from sport_team_i18n where sport_team_id =  hst.id)a1)::text host_team_name_json,

	 (SELECT array_to_json(array_agg(row_to_json(aa))) from (select sport_team_id as name,local as language from sport_team_i18n where sport_team_id =  gst.id)aa)::text guest_team_name_json
