...
CREATE OR REPLACE VIEW "v_sys_credit" AS
 SELECT ss.id,
    ...
    (SELECT si.value  FROM site_i18n si WHERE si.site_id = ss.id AND si.locale = ss.main_language::bpchar AND si.type::text = 'site_name'::text AND si.module::text = 'setting'::text AND si.key::text = 'name'::text LIMIT 1)::varchar(100) AS site_name,
    ...
    ( SELECT sum(cr.pay_amount) AS sum
           FROM credit_record cr WHERE cr.status::text = '2'::text  AND cr.pay_type::text = '1'::text
            AND cr.create_time >= ( SELECT date_trunc('month'::text, now() + replace(sys_site.timezone::text, 'GMT'::text, ''::text)::interval ) - replace(sys_site.timezone::text, 'GMT'::text, ''::text)::interval
                                       FROM sys_site WHERE sys_site.id = ss.id) AND cr.create_time <  ( SELECT date_trunc('month'::text, now() + replace(sys_site.timezone::text, 'GMT'::text, ''::text)::interval + '1 mon'::interval) - replace(sys_site.timezone::text, 'GMT'::text, ''::text)::interval
                                      FROM sys_site WHERE sys_site.id = ss.id) AND cr.site_id = ss.id
    ) AS all_recharge,
    ss.credit_line,
    ss.profit_time,
        CASE
