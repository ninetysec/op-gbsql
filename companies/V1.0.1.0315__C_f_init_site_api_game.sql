...
SELECT siteid,id, status, order_num, code, maintain_start_time,maintain_end_time,transferable,NULL,terminal,order_num
from api a WHERE NOT EXISTS (select * from site_api s WHERE a.id=s.api_id and s.site_id=siteid);
