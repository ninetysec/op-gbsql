-- auto gen by admin 2016-04-15 20:46:30
select redo_sqls($$
...
DELETE FROM site_i18n WHERE type='game_tag' AND key='new_game';
DELETE FROM game_tag WHERE tag_id='new_game';
...
SELECT siteid,game_id,tag_id FROM game_tag g WHERE NOT EXISTS(SELECT * FROM site_game_tag s WHERE g.game_id=s.game_id and s.site_id=siteid);
GET DIAGNOSTICS v_count = ROW_COUNT;
...
"jackpot_id" varchar(64) COLLATE "default"  NOT NULL,
"jackpot_name" varchar(128) COLLATE "default",
CONSTRAINT "game_jackpot_pkey" PRIMARY KEY ("id")
)
WITH (OIDS=FALSE);

COMMENT ON TABLE "game_jackpot" IS '游戏彩池表';

COMMENT ON COLUMN "game_jackpot"."id" IS '主键';

COMMENT ON COLUMN "game_jackpot"."game_id" IS '游戏ID';

COMMENT ON COLUMN "game_jackpot"."jackpot_id" IS '彩池id';

COMMENT ON COLUMN "game_jackpot"."jackpot_name" IS '彩池名称';

DROP index if EXISTS fk_game_jackpot_game_id;
CREATE INDEX "fk_game_jackpot_game_id" ON "game_jackpot" USING btree (game_id);

DROP index if EXISTS fk_game_jackpot_jackpot_id;
CREATE INDEX "fk_game_jackpot_jackpot_id" ON "game_jackpot" USING btree (jackpot_id);

select redo_sqls($$
			ALTER TABLE game_jackpot  ADD CONSTRAINT u_game_jackpot UNIQUE (game_id, jackpot_id);
$$);
