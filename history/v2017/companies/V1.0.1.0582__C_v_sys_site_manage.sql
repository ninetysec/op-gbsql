-- auto gen by linsen 2018-03-25 17:41:50
-- 修改站点管视图profit_percent除数为零的时候值零 by linsen
...
CREATE OR REPLACE VIEW "v_sys_site_manage" AS
 SELECT ss.id,
    ...
    ds.idc,
        CASE
            WHEN ((ss.max_profit + COALESCE(ss.credit_line)) = (0)::numeric) THEN (0)::numeric
            ELSE ((COALESCE(ss.has_use_profit) * (100)::numeric) / (ss.max_profit + COALESCE(ss.credit_line)))
