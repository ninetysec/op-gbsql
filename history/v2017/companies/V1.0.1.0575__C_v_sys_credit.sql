-- auto gen by linsen 2018-03-16 17:36:08
--修改买分管理视图的本月充值统计 by younger
...
CREATE OR REPLACE VIEW "v_sys_credit" AS
 SELECT ss.id,
    ...
    ( SELECT (COALESCE(sum(cr.pay_amount), (0)::numeric))::numeric(20,2) AS "coalesce"
          ...
          WHERE (((cr.status)::text = '2'::text) AND ((cr.pay_type)::text in ('1'::text,'4'::text)) AND (cr.create_time >= ( SELECT (date_trunc('month'::text, (now() + (replace((sys_site.timezone)::text, 'GMT'::text, ''::text))::interval)) - (replace((sys_site.timezone)::text, 'GMT'::text, ''::text))::interval)
                   FROM sys_site
