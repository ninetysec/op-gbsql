-- auto gen by linsen 2018-03-28 16:32:35
-- 修改买分管理视图当前转账上限 by linsen
...
CREATE OR REPLACE VIEW "v_sys_credit" AS
 SELECT ss.id,
    ...
    (COALESCE(ssc.transfer_out_sum, (0)::numeric) - COALESCE(ssc.transfer_into_sum, (0)::numeric)) AS transfer_sum,
        CASE
            WHEN ((COALESCE(ssc.current_transfer_limit, 0.01) + COALESCE(ssc.transfer_line, (0)::numeric)) = (0)::numeric) THEN (0)::numeric
            ELSE (((COALESCE((COALESCE(ssc.transfer_out_sum, (0)::numeric) - COALESCE(ssc.transfer_into_sum, (0)::numeric)), (0)::numeric) / (COALESCE(ssc.current_transfer_limit, 0.01) + COALESCE(ssc.transfer_line, (0)::numeric))) * (100)::numeric))::numeric(38,2)
