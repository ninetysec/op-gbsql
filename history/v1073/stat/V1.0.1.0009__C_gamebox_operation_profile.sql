-- auto gen by cheery 2016-03-04 10:07:02
...
BEGIN
	...
	DELETE FROM operation_profile WHERE site_id = sid AND to_char(statistics_time, 'yyyy-MM-dd') = sta_date;

	raise info '统计　%　号经营数据.START', sta_date;
	FOR rec IN
		SELECT s.operationid, s.masterid, s.siteid,
			   o.new_player, o.new_player_deposit, o.new_agent,
		  ...
		  FROM dblink (
			...
			'SELECT (SELECT gamebox_current_site()) 	as site_id,
					(SELECT COUNT(1) FROM sys_user
					  WHERE user_type = ''24''
					    AND to_char(create_time, ''yyyy-MM-dd'') = to_char('''||sta_date||'''::TIMESTAMP, ''yyyy-MM-dd'')) as new_player,
					(SELECT COUNT(1) FROM sys_user
					  WHERE user_type = ''23''
					  	AND to_char(create_time, ''yyyy-MM-dd'') = to_char('''||sta_date||'''::TIMESTAMP, ''yyyy-MM-dd'')) as new_agent,
					(SELECT COUNT(DISTINCT player_id)
					   FROM player_transaction
					  WHERE player_id IN (SELECT id FROM sys_user WHERE user_type = ''24'' AND to_char(create_time, ''yyyy-MM-dd'') = to_char('''||sta_date||'''::TIMESTAMP, ''yyyy-MM-dd''))
						AND to_char(create_time, ''yyyy-MM-dd'') = to_char('''||sta_date||'''::TIMESTAMP, ''yyyy-MM-dd'')
						AND transaction_type = ''deposit''
						AND status = ''success'') 		as new_player_deposit,
					(SELECT SUM(transaction_money)
					   FROM player_transaction
					  WHERE to_char(create_time, ''yyyy-MM-dd'') = to_char('''||sta_date||'''::TIMESTAMP, ''yyyy-MM-dd'')
						AND transaction_type = ''deposit''
					...
					(SELECT SUM(COALESCE(effective_trade_amount, 0.00))
					  ...
					  WHERE to_char(create_time, ''yyyy-MM-dd'') = to_char('''||sta_date||'''::TIMESTAMP, ''yyyy-MM-dd'')) as effective_transaction_volume,
					(SELECT COUNT(DISTINCT player_id)
					   FROM player_game_order
					  WHERE to_char(create_time, ''yyyy-MM-dd'') = to_char('''||sta_date||'''::TIMESTAMP, ''yyyy-MM-dd'')) as transaction_player,
					(SELECT SUM(COALESCE(profit_amount, 0.00))
					   FROM player_game_order
					  WHERE to_char(create_time, ''yyyy-MM-dd'') = to_char('''||sta_date||'''::TIMESTAMP, ''yyyy-MM-dd'')) as transaction_profit_loss')
	...
	LOOP
		INSERT INTO operation_profile (
			...
			sta_date::TIMESTAMP, rec.new_player, rec.new_player_deposit, rec.new_agent,
	...
	END LOOP;
	raise info '统计　%　号经营数据.END', sta_date;

END;

$$ language plpgsql;
COMMENT ON FUNCTION gamebox_operation_profile(dblink_url TEXT, sid INT, sta_date TEXT)
IS 'Fly-经营概况';