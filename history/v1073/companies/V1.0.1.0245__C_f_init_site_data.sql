-- auto gen by cherry 2017-01-21 11:49:20
...
--同步api到站点
INSERT INTO site_api (site_id,api_id,status,order_num,code,maintain_start_time,maintain_end_time,transferable,terminal)
SELECT siteid,id,status,order_num,code,maintain_start_time,maintain_end_time,transferable,terminal FROM api a WHERE a.id=apiid AND NOT EXISTS(SELECT * FROM site_api s WHERE a.id=s.api_id and s.site_id=siteid);
GET DIAGNOSTICS v_count = ROW_COUNT;
...
INSERT INTO site_game (site_id,game_id, api_id, game_type, order_num, url, status, api_type_id)
SELECT siteid,id, api_id, game_type, order_num, url, status,api_type_id from game g WHERE api_id=apiid AND NOT EXISTS (select * from site_game s WHERE g.id=s.game_id and s.site_id=siteid);
GET DIAGNOSTICS v_count = ROW_COUNT;
