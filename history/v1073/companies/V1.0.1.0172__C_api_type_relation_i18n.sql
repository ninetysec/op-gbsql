-- auto gen by cherry 2016-09-07 14:32:28
CREATE TABLE IF NOT EXISTS "api_type_relation_i18n" (
"id" int4 NOT NULL,
"api_id" int4,
"api_type_id" int4,
"name" varchar(100) COLLATE "default",
"local" char(5) COLLATE "default",
CONSTRAINT "api_type_relation_i18n_pkey" PRIMARY KEY ("id")
)
WITH (OIDS=FALSE)
;

COMMENT ON TABLE "api_type_relation_i18n" IS 'api_type_relation_i18n-- susu';

COMMENT ON COLUMN "api_type_relation_i18n"."id" IS '主键';

COMMENT ON COLUMN "api_type_relation_i18n"."api_id" IS 'api主键';

COMMENT ON COLUMN "api_type_relation_i18n"."api_type_id" IS 'api分类';

COMMENT ON COLUMN "api_type_relation_i18n"."name" IS 'API分类下默认API名称';

COMMENT ON COLUMN "api_type_relation_i18n"."local" IS '语言代码';
...
INSERT INTO site_api_type_relation_i18n (relation_id, name, local)
SELECT s.id,a.name,a.local FROM site_api_type_relation s,api_type_relation_i18n a WHERE s.api_id=a.api_id AND s.api_type_id=a.api_type_id AND site_id=siteid AND NOT EXISTS (SELECT * FROM site_api_type_relation_i18n si WHERE s.id=si.relation_id);
GET DIAGNOSTICS v_count = ROW_COUNT;
