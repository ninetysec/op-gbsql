...
SELECT siteid,id, apiid, game_type, order_num, url, status,api_type_id,support_terminal,code from game g WHERE NOT EXISTS (select * from site_game s WHERE g.id=s.game_id and s.site_id=siteid);
GET DIAGNOSTICS v_count = ROW_COUNT;
