-- auto gen by admin 2016-04-27 20:56:21
DROP VIEW IF EXISTS v_sys_user_operators;

DROP VIEW IF EXISTS v_site_master_manage;

ALTER TABLE "sys_user" ALTER COLUMN "constellation" TYPE varchar(15) COLLATE "default";
...
CREATE OR REPLACE VIEW v_sys_user_operators AS
 SELECT t2.name,
   ...
   FROM ((("gb-companies".sys_user t1
     LEFT JOIN "gb-companies".sys_site t2 ON ((t1.id = t2.sys_user_id)))
     LEFT JOIN "gb-companies".user_extend t3 ON ((t1.id = t3.id)))
     LEFT JOIN ( SELECT site_contract_scheme.center_id,
           ...
           FROM "gb-companies".site_contract_scheme
...
CREATE OR REPLACE VIEW  "v_site_master_manage" AS
 SELECT su.id,
   ...
   FROM ((("gb-companies".sys_user su
     LEFT JOIN "gb-companies".user_extend ue ON ((su.id = ue.id)))
     LEFT JOIN "gb-companies".sys_site ss ON ((su.site_id = ss.id)))
