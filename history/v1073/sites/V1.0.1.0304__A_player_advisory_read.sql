-- auto gen by cherry 2016-11-08 14:14:51
...
SELECT pa.id,
  ...
  FROM player_advisory pa
       ...
       LEFT JOIN (SELECT id,
                   ...
                   WHERE id = (SELECT max(id) FROM player_advisory_reply WHERE player_advisory_id = pari.id)
                 ) reg ON reg.player_advisory_id = pa.id;

COMMENT ON VIEW v_player_advisory IS '咨询视图-orange-Edit by Leisure';

 select redo_sqls($$
  ALTER TABLE player_game_order ADD COLUMN bet_type character varying(32);
	ALTER TABLE player_game_order ADD COLUMN bet_detail text;
$$);

COMMENT ON COLUMN player_game_order.bet_type IS '投注类型';
COMMENT ON COLUMN player_game_order.bet_detail IS '注单详情';