-- auto gen by cherry 2016-11-25 16:57:21
--玩家推荐新增层级、终端
			...
			LOOP
				...
				IF rewardWay = '1' THEN	-- 奖励双方
					...
					INSERT INTO player_transaction (
						...
						completion_time, fund_type, transaction_way, transaction_data,rank_id,origin)
					...
					VALUES (
						...
						now(), fundType, single, '{"username":"'||rec.berecommend_name||'","rewardType":"'||referee||'"}',(SELECT rank_id FROM user_player WHERE id = rec.recommend_id),pcOrigin);
					...
					VALUES (
						...
						now(), fundType, single, '{"username":"'||rec.recommend_name||'","rewardType":"'||referen||'"}',(SELECT rank_id FROM user_player WHERE id = rec.berecommend_id), pcOrigin);
				...
				LOOP
					...
					IF rec.recommend_total >= br.playernum THEN
						...
						VALUES (
							...
							now(), fundType, bonus, '{"username":"'||rec.recommend_name||'"}',(SELECT rank_id FROM user_player WHERE id = rec.recommend_id), pcOrigin);
