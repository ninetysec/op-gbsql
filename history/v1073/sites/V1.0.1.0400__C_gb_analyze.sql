-- auto gen by cherry 2017-03-07 14:45:49
	        ...
	        --存款
	         ...
	         SELECT player_id,
	          ...
	          WHERE recharge_status IN ('2', '5')
	            ...
	            AND check_time >= p_start_time AND check_time < p_end_time
...
COMMENT ON FUNCTION gb_analyze("p_stat_date" date, "p_start_time" timestamp, "p_end_time" timestamp) IS 'Leisure-经营分析-玩家';
...
INSERT INTO sys_resource ("id", "name", "url", "remark", "parent_id", "structure", "sort_num", "subsys_code", "permission", "resource_type", "icon", "built_in", "privilege", "status") SELECT '90101', '有效玩家设置', '', '', '9', '', '3', 'mcenter', 'mcenter:analyze-param', '2', '', 't', 'f', 't'

WHERE NOT EXISTS (SELECT id FROM sys_resource WHERE id=90101);