-- auto gen by cherry 2016-03-17 16:18:10
...
loop
	...
	SELECT order_column FROM activity_preferential_relation WHERE activity_message_id = activitymessageid AND preferential_code = type_effective_transaction AND preferential_value <= preferential_value_temp order by order_column desc LIMIT 1 into order_num;
		...
		loop
			...
			ELSE
				...
				WITH playtransaction AS (
					...
					(orderNo, now(), 'favorable', '系统自动计算玩家参与优惠活动【有效交易量】', rec_way.preferential_value, rec_way.preferential_value + (SELECT wallet_balance FROM user_player WHERE id = rec_apply.user_id), 'success',rec_apply.user_id, favorable_id_temp, auditPoints, false, false, 'favourable', activitytype, activityname_json)
...
DROP FUNCTION f_preferential_message(activitymessageid int4, sitecode varchar);
...
IF activitytype = type_profit_loss THEN
	...
	SELECT f_preferential_profitloss(activitymessageid,sitecode);

	RETURN;

ELSEIF activitytype = type_effective_transaction THEN

	SELECT f_preferential_effecttransaction(activitymessageid,sitecode);
