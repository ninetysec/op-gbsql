-- auto gen by cherry 2017-05-18 11:28:48
...
update player_game_order po set profit_amount=(COALESCE(lo.payout,0)-COALESCE(lo.bet_amount,0)),payout_time=lo.payout_time,effective_trade_amount=lo.effective_trade_amount,
		...
		update_time=now(),order_state='settle',is_profit_loss=lo.payout>0,result_json=(select array_to_json(array_agg(row_to_json(t))) json from (select * from lottery_bet_order o where o.id=lo.id) t)
