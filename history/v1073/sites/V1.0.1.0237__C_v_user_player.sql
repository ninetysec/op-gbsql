-- auto gen by cherry 2016-08-29 09:36:08
...
CREATE OR REPLACE VIEW "v_user_player" AS
 ...
 SELECT a.id,
    ...
    ((COALESCE(a.wallet_balance,0) + COALESCE(a.freezing_funds_balance,0)) + COALESCE(( SELECT sum(player_api.money) AS sum

           FROM player_api

          WHERE (player_api.player_id = a.id)),0)) AS total_assets,
