-- auto gen by tom 2016-03-16 16:01:40
CREATE OR REPLACE FUNCTION "f_player_recommend_award"(sitecode varchar, ordertype varchar)
	...
	IF singleReward THEN	-- 单次奖励
		FOR rec IN
			SELECT * FROM (
				...
				/* 被推荐人满足交易量 */
				SELECT su."id" berecommend_id, su.username berecommend_name, ep.recharge_amount
				  FROM (SELECT player_id, SUM(recharge_amount) recharge_amount FROM player_recharge GROUP BY player_id) ep
				  LEFT JOIN sys_user su ON ep.player_id = su."id"
				 WHERE su.user_type = '24' AND ep.recharge_amount >= deposit
			...
			LOOP
				...
				IF rewardWay = '1' THEN	-- 奖励双方
					...
					VALUES (orderNo, now(), transType, '推荐奖励-单次', rewardAmount, rewardAmount + (SELECT wallet_balance FROM user_player WHERE id = rec.recommend_id), transStatus,
						rec.recommend_id, (SELECT id FROM award1), rec.recharge_amount, rewardPoint, false, false, fundType, single, '{"username":"'||rec.berecommend_name||'","rewardType":"'||referen||'"}');
					-- 修改玩家余额
					...
					WITH award2 AS (
						...
						INSERT INTO player_recommend_award (transaction_no, user_id, user_name, recommend_user_id, recommend_user_name, reward_mode, reward_amount, reward_time, reward_reason)
						VALUES (orderNo,
							rec.berecommend_id, rec.recommend_name, rec.berecommend_id, rec.berecommend_name, single, rewardAmount, now(), '被推荐')
						RETURNING id
					)
					-- 新增交易订单数据
					INSERT INTO player_transaction (transaction_no, create_time, transaction_type, remark, transaction_money, balance, status, player_id, source_id,
					...
					VALUES (orderNo,
						...
						rec.berecommend_id, (SELECT id FROM award2), rec.recharge_amount, rewardPoint, false, false, fundType, single, '{"username":"'||rec.recommend_name||'","rewardType":"'||referee||'"}');
					-- 修改玩家余额
				...
				ELSEIF rewardWay = '2' THEN 	-- 奖励推荐人
					...
					INSERT INTO player_transaction (transaction_no, create_time, transaction_type, remark, transaction_money, balance, status, player_id, source_id,
						effective_transaction, favorable_audit_points, is_satisfy_audit, is_clear_audit, fund_type, transaction_way, transaction_data)
					VALUES (orderNo, now(), transType, '推荐奖励-单次', rewardAmount, rewardAmount + (SELECT wallet_balance FROM user_player WHERE id = rec.recommend_id), transStatus,
						rec.recommend_id, (SELECT id FROM award), rec.recharge_amount, rewardPoint, false, false, fundType, single, '{"username":"'||rec.berecommend_name||'","rewardType":"'||referen||'"}');
					-- 修改玩家余额
					UPDATE user_player SET wallet_balance = wallet_balance + rewardAmount WHERE id = rec.recommend_id;
				...
				ELSEIF rewardWay = '3' THEN	-- 奖励被推荐人
					...
					VALUES (orderNo,
						now(), transType, '推荐奖励-单次', rewardAmount, rewardAmount + (SELECT wallet_balance FROM user_player WHERE id = rec.berecommend_id), transStatus,
						rec.berecommend_id, (SELECT id FROM award), rec.recharge_amount, rewardPoint, false, false, fundType, single, '{"username":"'||rec.recommend_name||'","rewardType":"'||referee||'"}');
					-- 修改玩家余额
					UPDATE user_player SET wallet_balance = wallet_balance + rewardAmount WHERE id = rec.berecommend_id;
