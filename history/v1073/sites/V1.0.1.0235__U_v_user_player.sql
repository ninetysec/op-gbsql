-- auto gen by bruce 2016-08-28 14:47:53
select redo_sqls($$
  ALTER TABLE user_player_transfer ADD COLUMN bank_code varchar(50);
  ALTER TABLE user_player_transfer ADD COLUMN user_id int4;
  ALTER TABLE user_player ADD COLUMN import_username varchar(32);
  $$);
COMMENT ON COLUMN user_player_transfer.bank_code IS '银行代码';
COMMENT ON COLUMN user_player_transfer.user_id IS '用户ID';
COMMENT ON COLUMN user_player.import_username IS '导入的用户名';

drop view if EXISTS v_user_player;
CREATE OR REPLACE VIEW "v_user_player" AS
 SELECT a.id,
    a.rank_id,
    a.wallet_balance+a.freezing_funds_balance+(select sum(money) from player_api where player_id=a.id) total_assets,
    a.phone_code,
    a.wallet_balance,
