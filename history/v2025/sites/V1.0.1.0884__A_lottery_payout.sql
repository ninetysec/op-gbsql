-- auto gen by marz 2018-06-24 17:13:15
...
INSERT INTO lottery_transaction (user_id,username,transaction_type,money,balance,transaction_time,terminal,source_id,memo)
		SELECT  lbo.user_id ,lbo.username , '2', lbo.payout, ((select p.money from player_api p where p.player_id=lbo.user_id and p.api_id=22) + sum(payout) over(partition by lbo.user_id order by lbo.id)) as balance ,
...
update player_game_order po set profit_amount=(COALESCE(lo.payout,0)-COALESCE(lo.effective_trade_amount,0)),payout_time=lo.payout_time,effective_trade_amount=lo.effective_trade_amount,
		update_time=NOW_TIME,order_state='settle',is_profit_loss=((COALESCE(lo.payout,0)-COALESCE(lo.bet_amount,0))!=0),result_json=(select array_to_json(array_agg(row_to_json(t))) json from (select * from lottery_bet_order o where o.id=lo.id) t)
...
update player_game_order_detail pod set payout_time=lo.payout_time,order_state='settle',result_json=(select array_to_json(array_agg(row_to_json(t))) json from (select * from lottery_bet_order o where o.id=lo.id) t)
   FROM lottery_bet_order lo  WHERE pod.api_id=22 AND pod.bet_id=lo.id::VARCHAR AND lo.code=lottery_code AND lo.expect=lottery_expect AND lo.payout_time=NOW_TIME;
