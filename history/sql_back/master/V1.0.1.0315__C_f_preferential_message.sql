-- auto gen by cherry 2016-01-06 16:59:25
DROP FUNCTION IF EXISTS f_preferential_message(integer, character varying);

CREATE OR REPLACE FUNCTION f_preferential_message(activitymessageid integer, sitecode character varying)
  RETURNS void AS
$BODY$declare
...
raise info '该活动的是否需要审核状态为:%',isaudit;
--获取该活动的所有申请玩家
...
loop
activitytype = rec_apply.activity_type_code;
raise info '计算玩家能获得的优惠，玩家id为:%,活动类型为：%',rec_apply.user_id,activitytype;
--获取当前活动的优惠规则
...
raise info '当前统计值为:%',preferential_value_temp;
select order_column from activity_preferential_relation where activity_message_id = activitymessageid and preferential_code = rec_preferential_code.preferential_code and preferential_value <= preferential_value_temp order by order_column desc LIMIT 1 into order_num_temp;
	if COALESCE (order_num_temp, 0) < order_num then
	order_num = order_num_temp;
	end if;
...
WITH playtransaction AS (
	...
	rec_way.preferential_value + (SELECT wallet_balance FROM user_player WHERE id = rec_apply.user_id), 'success',rec_apply.user_id, favorable_id_temp, rec_way.preferential_value*rec_way.preferential_audit, false, false, 'favourable', activitytype, '{"favorableType":"'||activitytype||'"}')
						RETURNING id
					)
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION f_preferential_message(integer, character varying)
  OWNER TO postgres;
COMMENT ON FUNCTION f_preferential_message(integer, character varying) IS '优惠活动计算入口';