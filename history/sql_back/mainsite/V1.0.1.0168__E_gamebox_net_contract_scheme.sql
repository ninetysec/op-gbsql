-- auto gen by Lins 2015-12-22 09:06:59

--   select gamebox_contract(1,TRUE);
--   select gamebox_contract(1,FALSE);
...
BEGIN
	sql='select a.contract_scheme_id,a.id
		...
		where a.id=b.contract_occupy_grads_id and b.ratio is not null
	...
	sql=sql||'
		...
		where a1.id=b1.contract_occupy_grads_id
			AND a1.contract_scheme_id=a.contract_scheme_id and b1.ratio is not null
...
BEGIN
	...
	LOOP
		...
		val=min_value||cs||max_value||cs||favourable_type||cs||favourable_way
		||cs||favourable_value||cs||favourable_limit;
