-- auto gen by linsen 2018-05-08 09:08:53
-- 优惠活动申请统一入口 by kobe
...
declare
  ...
  sitecode varchar;--站点code
	applyhash VARCHAR; --3秒内一个玩家一个活动的唯一标识,防止并发
...
BEGIN
  ...
  IF activitytype = TYPE_REGIST_SEND OR activitytype =TYPE_RELIEF_FUND THEN
    ...
    preferentialdata=ruleresult->>'preferentialdata';
  END IF;
  orderNo := (SELECT f_order_no(sitecode, '02'));
  SELECT (EXTRACT(EPOCH FROM NOW())/3)::BIGINT INTO applyhash;
	applyhash = applyhash || playerid || activitymessageid;
IF  NOT EXISTS (SELECT * FROM activity_player_apply WHERE user_id=playerid AND activity_message_id=activitymessageid AND start_time=applystarttime )  THEN
  WITH activityaplayapply AS (
			INSERT INTO activity_player_apply(activity_message_id,user_id,user_name,register_time,rank_id,rank_name,risk_marker,apply_time,check_state,start_time,end_time,ip_apply,ip_dict_code,activity_terminal_type,apply_transaction_no,apply_hash)
			VALUES (activitymessageid,playerid,username,registertime,rankid,rankname,riskmarker,applytime,APPLY_CHECK_STATUS_CONFIRMED,applystarttime,applyendtime,loginIP,loginIpDictCode,terminaltype,orderNo,applyhash)
			RETURNING id
  )
 SELECT id FROM activityaplayapply INTO applyid ;
  RETURN json_build_object( 'resultcode',result_code,'applyId',applyid,'ordernum',ordernum,'preferentialdata',preferentialdata);
ELSE
