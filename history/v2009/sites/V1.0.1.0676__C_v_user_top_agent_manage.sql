-- auto gen by george 2018-01-25 20:22:44
...
CREATE OR REPLACE VIEW "v_user_top_agent_manage" AS
 SELECT ua.id,
    ...
    ( SELECT count(1) AS count
           FROM user_agent ua1,
            sys_user su_1
          WHERE ((ua1.id = su_1.id) AND ((su_1.user_type)::text = '23'::text) AND (ua1.parent_id = ua.id) AND (ua1.agent_rank = 1) AND ((su_1.status)::text < '5'::text))) AS child_agent_num,
    ( SELECT count(1) AS count
           FROM sys_user su1
          WHERE ((su1.owner_id IN ( SELECT su2.id
                   FROM user_agent ua1,
                    sys_user su2
                  WHERE ((ua1.id = su2.id) AND (ua1.parent_id = ua.id)))) AND ((su1.status)::text < '5'::text) AND ((su1.user_type)::text = '24'::text))) AS player_num,
    ( SELECT count(1) AS count
           FROM (rebate_set rs
             JOIN user_topagent_rebate uar ON ((uar.rebate_id = rs.id)))
