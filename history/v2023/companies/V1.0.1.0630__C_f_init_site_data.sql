-- auto gen by cherry 2018-06-02 09:32:34
...
INSERT INTO site_api_type_i18n (site_id, api_type_id, name, local, cover,mobile_name)
SELECT siteid, api_type_id, name, local, cover,mobile_name from api_type_i18n a WHERE NOT EXISTS (select * from site_api_type_i18n s WHERE a.api_type_id=s.api_type_id and s.site_id=siteid);
GET DIAGNOSTICS v_count = ROW_COUNT;
raise notice '新增site_api_type_i18n数量 %', v_count;

INSERT INTO site_api_type_relation (site_id, api_id, api_type_id,order_num,mobile_order_num)
SELECT siteid, api_id, api_type_id,order_num,order_num from api_type_relation a WHERE NOT EXISTS (SELECT * FROM site_api_type_relation s WHERE a.api_id = s.api_id AND a.api_type_id = s.api_type_id AND s.site_id=siteid);
GET DIAGNOSTICS v_count = ROW_COUNT;
...
INSERT INTO site_i18n ( "module", "type", "key", "locale", "value", "site_id","default_value","built_in") SELECT
  module,type,key,locale,value,siteid,default_value,FALSE FROM   site_i18n WHERE  site_id = 0 AND module = 'setting' AND (type = 'service_terms' OR "type"='service_terms_agent')AND built_in = TRUE AND NOT
 EXISTS (SELECT * from site_i18n where site_id = siteid AND  module = 'setting' AND (type = 'service_terms' OR "type"='service_terms_agent'));
GET DIAGNOSTICS v_count = ROW_COUNT;
...
raise notice '开始同步彩票赔率';
perform  f_init_lottery_odd(siteid);

raise notice '开始同步彩票限额';
...
COMMENT ON FUNCTION "f_init_site_data"(siteid int4) IS '同步站点数据';
