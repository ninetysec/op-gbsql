-- auto gen by steffan 2018-05-28 11:17:13
...
BEGIN
  ...
  --生成优惠活动申请
  SELECT * FROM gamebox_activityhall_apply(activitymessage ,playerInfo, terminaltype) INTO applyresult;
  ...
  END IF;
  --计算活动优惠
  SELECT * FROM gamebox_activityhall_calculator(activitymessage, playerinfo, applyresult,terminaltype) INTO resultjson;
...
COMMENT ON FUNCTION "gamebox_activityhall_entrance"(activitymessageid int4, playerid int4, sitecode varchar, terminaltype varchar) IS '优惠活动统一入口';