-- auto gen by linsen 2018-08-08 11:47:38
-- 添加推荐人的username和user_type  by linsen
...
CREATE OR REPLACE VIEW  "v_user_player" AS
 SELECT a.id,
    ...
    ( SELECT re.username
           FROM sys_user re
          WHERE (re.id = a.recommend_user_id)) AS recommend_username,
		( SELECT re.user_type
           FROM sys_user re
          WHERE (re.id = a.recommend_user_id)) AS recommend_user_type,
    a.import_username,
