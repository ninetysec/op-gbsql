-- auto gen by marz 2018-07-15 11:36:44
		...
		INSERT INTO lottery_transaction (user_id,username,transaction_type,money,balance,transaction_time,terminal,source_id,memo)
			SELECT  lbo.user_id ,lbo.username , '5', trunc(lbo.rebate*lbo.bet_amount,3), ((select p.money from player_api p where p.player_id=lbo.user_id and p.api_id=22 FOR UPDATE) + sum(trunc(lbo.rebate*lbo.bet_amount,3)) over(partition by lbo.user_id order by lbo.id)) as balance ,
				lbo.payout_time,lbo.terminal,lbo.id,'投注下单返点' from lottery_bet_order lbo WHERE lbo.expect = lottery_expect and lbo.code = lottery_code and lbo.status='2' AND lbo.rebate>0
																																										and NOT EXISTS (SELECT t.id FROM lottery_transaction t WHERE t.source_id=lbo.id  AND t.transaction_type ='5') order by  lbo.user_id ,lbo.id;
		--返点更新钱包
		UPDATE player_api u SET money = COALESCE(money,0) + COALESCE( (SELECT SUM(trunc(lbo.rebate*lbo.bet_amount,3)) from lottery_bet_order lbo WHERE lbo.expect = lottery_expect and lbo.code = lottery_code and  u.player_id=lbo.user_id AND status='2' AND rebate>0 and lbo.payout_time = NOW_TIME) ,0)
		WHERE u.api_id=22
					and  EXISTS ( SELECT 1 from lottery_bet_order lbo WHERE lbo.expect = lottery_expect and lbo.code = lottery_code AND status='2' AND rebate>0 and lbo.payout_time = NOW_TIME and  lbo.user_id   =  u.player_id )  ;
